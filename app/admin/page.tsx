...
export default function AdminPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const gift: Gift = {
      ...
      categories: {
        gender: formData.gender ? formData.gender.split(',').map(s => s.trim()) : undefined,
        ageRange: formData.ageMin && formData.ageMax
          ? { min: parseInt(formData.ageMin), max: parseInt(formData.ageMax) }
          : undefined,
        nationalities: formData.nationalities
          ? formData.nationalities.split(',').map(s => s.trim())
          : undefined,
        jobs: formData.jobs
          ? formData.jobs.split(',').map(s => s.trim())
          : undefined,
      },
    }
